...
export function NodeDetailPanel({ nodeId, onClose, onSelectParent, standalone = false }: {
  ...
  if (!detail?.node) return <div className={`${standalone ? "" : "fixed inset-y-16 right-0 z-40 w-full max-w-xl"} glass grid place-items-center p-8 text-slate-400`}>载入节点…</div>;
  const { node } = detail;
  ...
  const recordSection = <section className="mt-7 rounded-2xl border border-white/10 bg-black/10 p-4"><h2 className="font-semibold">记述 · {detail.descriptions.length}</h2><p className="mt-1 text-xs text-slate-500">追加式公共记述，不参与遗传、Token 或图片生成。</p><form onSubmit={addDescription} className="mt-3 space-y-2"><textarea aria-label="记述" required maxLength={500} value={body} onChange={(event) => setBody(event.target.value)} placeholder="为这个实体追加纯文本记述" className="h-24 w-full rounded-xl border border-white/10 bg-slate-950 p-3 text-sm"/><input aria-label="署名" maxLength={64} value={authorLabel} onChange={(event) => setAuthorLabel(event.target.value)} placeholder="可选署名" className="w-full rounded-xl border border-white/10 bg-slate-950 p-3 text-sm"/><button className="rounded-xl bg-white px-4 py-2 text-sm font-medium text-slate-950">追加记述</button></form>{message && <p className="mt-3 text-xs text-cyan-300">{message}</p>}<div className="mt-4 space-y-2">{detail.descriptions.map((item) => <article key={item.id} className="rounded-xl border border-white/10 p-3 text-sm"><p className="whitespace-pre-wrap">{item.body}</p><p className="mt-2 text-xs text-slate-500">{item.authorLabel || "匿名访问者"} · {new Date(item.createdAt).toLocaleString()}</p></article>)}</div></section>;
  return <aside className={`${standalone ? "rounded-3xl" : "fixed inset-y-16 right-0 z-40 w-full max-w-xl border-l"} glass overflow-y-auto p-6 shadow-2xl`} data-testid="node-detail">
    <div className="flex items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[.22em] text-cyan-300">{node.type === "GENESIS" ? "Genesis root" : `Generation ${node.generation}`} · {node.protocolVersion}</p><h1 className="mt-1 text-3xl font-semibold">{node.name}</h1><p className="mt-1 text-xs text-slate-500">名称与身份不可修改 · {new Date(node.createdAt).toLocaleString()}</p></div>{onClose && <button onClick={onClose} aria-label="关闭" className="rounded-full border border-white/10 px-3 py-2">×</button>}</div>
    ...
    <div className="mt-5 flex gap-2">{onSelectParent && <button onClick={() => onSelectParent(node.id)} className="rounded-xl bg-fuchsia-500 px-4 py-2 text-sm font-medium">选择为亲本</button>} {!standalone && <Link href={`/nodes/${node.id}`} className="rounded-xl border border-white/10 px-4 py-2 text-sm">打开完整页面</Link>}</div>
    ...
    <section className="mt-7"><div className="flex items-center justify-between"><h2 className="font-semibold">实体设定 · 4 组{pairedEntities ? "双 Token" : "锚点"}</h2><span className="text-xs text-slate-500">{pairedEntities ? "主体 0+1 · 辅助 8+9 / 16+17 / 24+25" : "主体 0 · 辅助 8 / 16 / 24"}</span></div><p className="mt-2 text-xs leading-5 text-slate-500">{pairedEntities ? (node.promptVersion === "eros-entity-prompt-v8" ? "主体的 8-bit 编号按区间直接索引：0–63 现实自然生物、64–127 自然非生物、128–255 幻想种族；辅助实体使用基础元素与显现方式组合。伴随 Token 的其余 12 bit 定义气质、尺度和装束。" : node.promptVersion === "eros-entity-prompt-v7" ? "主体由两个最高 nibble 组成 8-bit 编号，直接索引 256 种明确物种；辅助实体使用基础元素与显现方式组合。伴随 Token 的其余 12 bit 定义气质、尺度和装束。" : "每组第一个 Token 定义基础实体与锚点特征，第二个 Token 定义形态或显现方式、气质、尺度与装束。") : "实体名称由每个锚点的最高 4 bit 决定；其余 12 bit 作为锚点附带特征单独展示。"}</p><div className="mt-3 grid grid-cols-2 gap-3">{entityTokens.map((token) => <EntityTokenCard key={token.position} token={token} />)}</div></section>
    <details className="mt-7 rounded-2xl border border-white/10 p-4"><summary className="cursor-pointer list-none"><div className="flex items-center justify-between"><h2 className="font-semibold">内在特征 · 去重视图</h2><span className="text-xs text-slate-500">展开查看 · {visibleDescriptorTokens.length} 个独立 Family</span></div></summary><p className="mt-3 text-xs leading-5 text-slate-500">按 genome 原始顺序扫描，同一 family 只优先展示 Token 序号最前的一条。</p><div className="mt-3 grid grid-cols-2 gap-2">{visibleDescriptorTokens.map((token) => <DescriptorTokenCard key={token.position} token={token} />)}</div><details className="mt-4 rounded-xl border border-white/10 p-3"><summary className="cursor-pointer text-xs text-cyan-300">查看全部 {descriptorTokens.length} 个原始内在 Token</summary><div className="mt-3 grid grid-cols-2 gap-2">{descriptorTokens.map((token) => <DescriptorTokenCard key={token.position} token={token} />)}</div></details></details>
    <details className="mt-7 rounded-2xl border border-white/10 p-4"><summary className="cursor-pointer list-none"><div className="flex items-center justify-between"><h2 className="font-semibold">Token 派生的确定性 Prompt</h2><span className="rounded-full border border-cyan-400/20 px-2 py-1 text-[10px] text-cyan-300">展开查看 · 仅服务端使用</span></div></summary><p className="mt-3 text-xs leading-5 text-slate-500">主体与三个辅助实体始终完整保留。仅对 {descriptorTokens.length} 个内在特征的 family 键去重，例如 contour、repetition；同一键只保留 Token 序号最小的整条特征。</p><pre data-testid="entity-prompt" className="mt-3 whitespace-pre-wrap rounded-xl bg-black/25 p-4 text-xs leading-5 text-slate-400">{detail.prompt}</pre></details>
    <section className="mt-7"><div className="flex items-center justify-between"><div><h2 className="font-semibold">视觉解释 · {completedImages.length}</h2><p className="mt-1 text-[10px] text-slate-500">FLUX.2 Klein 9B Preview · 保留模型原始尺寸</p></div><button onClick={addImage} className="rounded-xl border border-cyan-400/30 px-3 py-2 text-xs text-cyan-200">生成新解释</button></div><div className="mt-3 grid grid-cols-2 gap-2">{completedImages.map((image) => <figure key={image.id} className="overflow-hidden rounded-xl border border-white/10 bg-black"><img src={image.imageDataUrl ?? image.imageUrl} alt="视觉解释" className="h-auto w-full object-contain"/><figcaption className="px-2 py-1 text-center text-[9px] text-slate-600">{image.width && image.height ? `${image.width}×${image.height}` : "原始尺寸"}</figcaption></figure>)}</div></section>
    <div className="h-12" />
  </aside>;
}
