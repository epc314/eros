...
import { setHostedDescriptionFeedback } from "@/lib/hosted/repository";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    enforceRateLimit(`feedback:${requestAddress(request)}:${id}`, 30, 60_000);
    const input = schema.parse(await request.json());
    if (!/^[a-zA-Z0-9-]{8,128}$/.test(input.voterKey))
      throw new ApiFailure("INVALID_VOTER_KEY", "匿名评价标识无效。");
    return NextResponse.json(await setHostedDescriptionFeedback(id, input.voterKey, input.isTrue));
