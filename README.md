...
512-bit genome 被精确拆成两条 256-bit 染色体。`eros-v2` 使用 SHAKE-256 在两种互补配对间确定性选择：`parentLow.C0 + parentHigh.C1` 或 `parentHigh.C0 + parentLow.C1`。每个亲本恰好贡献一条，C0 永远保持在子代 C0，C1 永远保持在子代 C1。未选中的染色体仍参与突变排名种子。这样未突变的主体 Token 0 必然来自某个亲本的主体 Token 0，不会由辅助槽重新解释。
...
所以普通无关 Hash 保持很低的翻转量，更相似的节点单调地获得更多翻转。具体位置不是随机抽取：每个候选 bit 通过带域标签的 SHAKE-256 排名；排名种子包括规范化亲本、子代名称和两条未选染色体。按排名字节升序选择预算内的位置，再对基础 genome XOR。算法不读取时间、不使用随机数，也不依赖请求顺序，所以相同输入总是得到相同 Hash、Token 与 Prompt。
...
当前遗传协议是 `eros-v2`，图片模板是 `eros-entity-prompt-v8`。v8 将 256 项明确主体表严格分为 64 个现实自然生物、64 个自然非生物和 128 个幻想种族；v7 首次使用 8-bit 明确主体索引；v6 引入四组双 Token 实体编码；v5 使用审美安全、全局不重复的内在特征词表；v4 及更早节点继续使用冻结的旧解释，避免历史语义漂移。`eros-v1` 历史节点保持不可变；v2 修正了染色体换位导致主体槽语义漂移的问题。Hash 域标签、输入序列化、亲本规范化、染色体选择、曲线或突变排名的任何语义变化都必须发布新的协议版本，不能原地改变既有节点的解释。数据库在每个 World、Node 和图片记录中保存对应版本，以便未来版本共存和审计。
